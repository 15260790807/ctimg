a:7:{i:0;s:3141:"<?php $menuLists = $this->acl->getActiveResource(); ?><?php $this->_macros['findParent'] = function($__p = null) { if (isset($__p[0])) { $menus = $__p[0]; } else { if (isset($__p["menus"])) { $menus = $__p["menus"]; } else {  throw new \Phalcon\Mvc\View\Exception("Macro 'findParent' was called without parameter: menus");  } } if (isset($__p[1])) { $currentItem = $__p[1]; } else { if (isset($__p["currentItem"])) { $currentItem = $__p["currentItem"]; } else {  throw new \Phalcon\Mvc\View\Exception("Macro 'findParent' was called without parameter: currentItem");  } }  ?>
    <?php $lists = []; ?>
